...
struct fx_flac {
	...
	/**
	 * Number of channels in the output stream.  If so, mirrors number of channels in input stream.
	 */
	...
	/**
	 * Flags associated with the deocder.
	 *  0x01 - add blend, if set, output will be added into the buffer, otherwise buffer is overwritten
	 */
	uint8_t flags;
...
static bool _fx_flac_process_decoded_frame(fx_flac_t *inst, int16_t *out,
	...
	const uint8_t out_cc = (inst->out_channels == 0) ? cc : inst->out_channels;
	uint32_t n_smpls_rem =
	    (fh->block_size - inst->blk_cur - 1U) * out_cc + (out_cc - inst->chan_cur);
	...
	while (tar < n_smpls_rem) {
		/* if we are on a channel beyond the input channel, then re-use the first channel */
		inp_ch = (inst->chan_cur >= cc) ? 0 : inst->chan_cur;
...
void fx_flac_output_channels(fx_flac_t* inst, uint8_t output_channels)
{
	inst = (fx_flac_t*)FX_ALIGN_ADDR(inst);
	if (output_channels > inst->max_channels) {
		inst->out_channels = 0;
	} else {
		inst->out_channels = output_channels;
	}
}

void fx_flac_set_flag(fx_flac_t* inst, uint8_t flag, uint8_t value)
{
	inst = (fx_flac_t*)FX_ALIGN_ADDR(inst);
	if (value) {
		inst->flags |= (1 << flag);
