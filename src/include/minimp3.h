...
void mp3dec_decode(mp3dec_t* dec, const uint8_t* mp3, unsigned int* mp3_bytes, mp3d_sample_t* pcm, unsigned int* pcm_samples);
void mp3dec_set_output_channels(mp3dec_t* dec, int num_output_channels);
void mp3dec_set_flag(mp3dec_t* dec, unsigned int flag, int value);
...
int mp3dec_decode_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    if (!frame_size)
    {
        memset(dec, 0, sizeof(mp3dec_t) - sizeof(mp3dec_config_t));
...
void mp3dec_set_output_channels(mp3dec_t* dec, int num_output_channels)
{
    dec->cfg.num_output_channels = num_output_channels;
}
...
unsigned int mp3dec_copy_frame(mp3dec_t* dec, mp3d_sample_t* pcm, unsigned int pcm_samples)
{
    unsigned int copy_samples = 0;
    int num_output_channels = (dec->cfg.num_output_channels == 0) ? dec->info.channels : dec->cfg.num_output_channels;
    if (dec->frame_left) {
        unsigned int input_samples = dec->frame_left / dec->info.channels;
        copy_samples = pcm_samples / num_output_channels;
        copy_samples = (copy_samples < input_samples) ? copy_samples : input_samples;
        unsigned int s, ch, in_ch, frame_index = MINIMP3_MAX_SAMPLES_PER_FRAME - dec->frame_left;;
        for (s = 0; s < copy_samples; s++) {
            for (ch = 0; ch < num_output_channels; ch++) {
                in_ch = (ch >= dec->info.channels) ? 0 : ch;
                if (dec->cfg.flag & (1 << MINIMP3_FLAG_BLEND)) {
                    int16_t a = pcm[s * num_output_channels + ch];
                    int16_t b = dec->frame[frame_index + in_ch];
                    pcm[s * num_output_channels + ch] = k3_dsp_blend_sample16(a, b);
                } else {
                    pcm[s * num_output_channels + ch] = dec->frame[frame_index + in_ch];
                }
            }
            frame_index += dec->info.channels;
        }
        dec->frame_left -= copy_samples * dec->info.channels;
    }
    return copy_samples * num_output_channels;
}

void mp3dec_decode(mp3dec_t* dec, const uint8_t* mp3, unsigned int* mp3_bytes, mp3d_sample_t* pcm, unsigned int* pcm_samples)
{
    unsigned int copy_samples = mp3dec_copy_frame(dec, pcm, *pcm_samples);
    if (dec->frame_left == 0 && *mp3_bytes != 0) {
        dec->frame_left = mp3dec_decode_frame(dec, mp3, *mp3_bytes, dec->frame, &dec->info);
        copy_samples += mp3dec_copy_frame(dec, pcm + copy_samples, *pcm_samples - copy_samples);
        *mp3_bytes = dec->info.frame_bytes;
    } else {
        *mp3_bytes = 0;
    }
    *pcm_samples = copy_samples;
}
